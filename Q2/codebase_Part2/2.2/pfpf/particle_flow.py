...
from .optimal_schedule import get_optimal_schedule
...
def particle_flow(
    ...
    # --- Check for Optimal Beta (Dai 22) ---
    if ps["setup"].PFPF and getattr(ps["setup"], "optimal_beta", False):
        # Calculate optimal lambda range
        PP = np.asarray(vg["PP"])
        
        # J depends on mean.
        xp_m = np.asarray(vg["xp_m"]).reshape(-1, 1)
        likeparams = ps["likeparams"]
        # Jacobian H at the mean
        H = likeparams["dh_dx_func"](xp_m, likeparams)
        
        R = np.asarray(likeparams["R"])
        if R.ndim == 3: 
            R = R[:, :, 0] # Assume constant R approx for schedule calculation
        
        n_steps = len(ps["setup"].lambda_range) - 1
        if n_steps < 1: n_steps = 20
            
        opt_lambdas = get_optimal_schedule(PP, H, R, n_steps) 
        ps["setup"].lambda_range = opt_lambdas
