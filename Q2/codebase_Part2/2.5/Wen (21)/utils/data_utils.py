...
def load_data(data_path='../data/100s', filename='nav01_train', steps_per_episode=100, num_episodes=None):
    ...
    data = dict(np.load(os.path.join(data_path, filename + '.npz'),allow_pickle=True))
    # reshape data
...
## normal batch iterator
def make_batch_iterator(data, batch_size=32):
    datalen=data['s'].shape[0]
    seqlen=data['s'].shape[1]
    while True:
        index=np.random.choice(datalen,size=(datalen//batch_size,batch_size),replace=False)
        for i in range(datalen//batch_size):
            batches={k:data[k][index[i]] for k in data.keys()}
            yield batches

def make_repeating_batch_iterator(data, batch_size=32):
    datalen=data['s'].shape[0]
    seqlen=data['s'].shape[1]
    while True:
        for i in range(datalen//batch_size):
            batches={k:data[k][i*batch_size:(i+1)*batch_size] for k in data.keys()}
            yield batches
