"""SPFSM: Stochastic Particle Flow Filter with stiffness mitigation."""
...
class SPFSM:
	"""Stochastic particle-flow baseline with adaptive drift scaling."""
	...
	def _transition_sample(self, particles: tf.Tensor, time_step: tf.Tensor, seed: int) -> tf.Tensor:
		...
		noise = tf.random.stateless_normal(tf.shape(particles), seed=(seed, 51), stddev=tf.sqrt(self.process_var), dtype=particles.dtype)
		return drift + noise
	...
	def _flow_update(self, particles: tf.Tensor, observation: tf.Tensor, seed: int) -> tf.Tensor:
		x = particles
		lam = tf.cast(self.flow_step_size, x.dtype)
		sigma = tf.cast(self.diffusion_scale, x.dtype)
		for k in range(self.flow_steps):
			...
			drift_scale = tf.minimum(1.0, 10.0 / (grad_norm + 1e-6))
			noise = tf.random.stateless_normal(tf.shape(x), seed=(seed, 520 + k), stddev=sigma * tf.sqrt(lam), dtype=x.dtype)
			x = x + 0.5 * lam * drift_scale * grad + noise
		return x

	def initialize(self, seed: int) -> tuple[tf.Tensor, tf.Tensor]:
		particles = tf.random.stateless_normal((self.num_particles, self.state_dim), seed=(seed, 5), stddev=tf.sqrt(self.init_var), dtype=tf.float32)
	...
	def step(self, particles: tf.Tensor, log_weights: tf.Tensor, observation: tf.Tensor, time_step: tf.Tensor, seed: int) -> tuple[tf.Tensor, tf.Tensor, tf.Tensor, tf.Tensor]:
		...
		ess = self._ess(logw)
		x_new = self._flow_update(particles, observation, seed=seed)
