"""DPFHS: Differentiable Particle Filter with hard systematic resampling."""

from __future__ import annotations

import tensorflow as tf

from dataset import observation_mean_tf, transition_drift_tf


class DPFHS:
	"""Hard-resampling particle filter baseline.

	This implementation keeps standard non-differentiable systematic resampling
	with an ESS trigger, matching the hard-resampling DPF setting.
	"""

	def __init__(
		self,
		state_dim: int,
		num_particles: int,
		process_var: float,
		obs_var: float,
		init_var: float,
		ess_ratio: float = 0.5,
	) -> None:
		self.state_dim = int(state_dim)
		self.num_particles = int(num_particles)
		self.process_var = tf.constant(process_var, dtype=tf.float32)
		self.obs_var = tf.constant(obs_var, dtype=tf.float32)
		self.init_var = tf.constant(init_var, dtype=tf.float32)
		self.ess_threshold = tf.constant(ess_ratio * num_particles, dtype=tf.float32)

	@staticmethod
	def _normalize_logw(logw: tf.Tensor) -> tf.Tensor:
		return logw - tf.reduce_logsumexp(logw)

	@staticmethod
	def _ess(logw: tf.Tensor) -> tf.Tensor:
		return tf.exp(-tf.reduce_logsumexp(2.0 * logw))

	def _transition_sample(self, particles: tf.Tensor, time_step: tf.Tensor, seed: int) -> tf.Tensor:
		drift = transition_drift_tf(particles, time_step)
		noise = tf.random.stateless_normal(
			shape=tf.shape(particles),
			seed=(seed, 11),
			mean=0.0,
			stddev=tf.sqrt(self.process_var),
			dtype=particles.dtype,
		)
		return drift + noise

	def _observation_log_prob(self, particles: tf.Tensor, observation: tf.Tensor) -> tf.Tensor:
		loc = observation_mean_tf(particles)
		log_prob_dim = -0.5 * (
			tf.math.log(2.0 * tf.constant(3.141592653589793, dtype=particles.dtype) * self.obs_var)
			+ tf.square(observation[tf.newaxis, :] - loc) / self.obs_var
		)
		return tf.reduce_sum(log_prob_dim, axis=-1)

	def _systematic_indices(self, weights: tf.Tensor, seed: int) -> tf.Tensor:
		n = tf.shape(weights)[0]
		n_f = tf.cast(n, weights.dtype)
		cdf = tf.cumsum(weights)
		u0 = tf.random.stateless_uniform((), seed=(seed, 17), minval=0.0, maxval=1.0 / n_f, dtype=weights.dtype)
		positions = u0 + tf.range(n, dtype=weights.dtype) / n_f
		return tf.searchsorted(cdf, positions, side="right")

	def initialize(self, seed: int) -> tuple[tf.Tensor, tf.Tensor]:
		"""Initialize particles from Gaussian prior and uniform log-weights."""
		particles = tf.random.stateless_normal(
			shape=(self.num_particles, self.state_dim),
			seed=(seed, 1),
			mean=0.0,
			stddev=tf.sqrt(self.init_var),
			dtype=tf.float32,
		)
		logw = tf.fill([self.num_particles], -tf.math.log(tf.cast(self.num_particles, tf.float32)))
		return particles, logw

	def step(
		self,
		particles: tf.Tensor,
		log_weights: tf.Tensor,
		observation: tf.Tensor,
		time_step: tf.Tensor,
		seed: int,
	) -> tuple[tf.Tensor, tf.Tensor, tf.Tensor, tf.Tensor]:
		"""Run one predict-update-resample step."""
		particles = self._transition_sample(particles, time_step, seed=seed)
		logw = self._normalize_logw(log_weights + self._observation_log_prob(particles, observation))
		ess = self._ess(logw)
		do_resample = ess < self.ess_threshold

		def _resample() -> tuple[tf.Tensor, tf.Tensor]:
			indices = self._systematic_indices(tf.exp(logw), seed=seed)
			x_new = tf.gather(particles, indices)
			lw_new = tf.fill([self.num_particles], -tf.math.log(tf.cast(self.num_particles, tf.float32)))
			return tf.stop_gradient(x_new), lw_new

		particles, logw = tf.cond(do_resample, _resample, lambda: (particles, logw))
		return particles, logw, ess, tf.cast(do_resample, tf.float32)
